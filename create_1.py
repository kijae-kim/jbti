...
openai.api_key ="sk-None-n8HiWJNZXNK1Ee6EwGDbT3BlbkFJfDTiUlsmvocdwugsX7Yy"
...
# 임베딩 로드 함수
def load_embeddings_from_mongodb(client, db_name, embed_type):
    try:
        db = client[db_name]
        collection = db['embeddings']
        data = list(collection.find({"type": embed_type}))
        embeddings = np.array([d["embedding"] for d in data])
        logging.info(f"{embed_type} 임베딩 로드 성공")
        return embeddings
    except Exception as e:
        logging.error(f"{embed_type} 임베딩 로드 실패: {e}")
        raise e
...
# 최종 결과 생성 함수
def final_results(user_mbti, user_job, mbti_detail_df, holland_result_df, job_detail_df, results, client, config, ttl_seconds, mbti_embeddings, job_embeddings):
...
def main(user_mbti, user_job):
    try:
        ...
        # 사전 저장된 임베딩 로드
        mbti_embeddings = load_embeddings_from_mongodb(client, config['mongodb']['db_name'], 'mbti')
        holland_embeddings = load_embeddings_from_mongodb(client, config['mongodb']['db_name'], 'holland')
        job_embeddings = load_embeddings_from_mongodb(client, config['mongodb']['db_name'], 'job')
        ...
        ttl_seconds = 3600
        initial_response, additional_response = final_results(user_mbti, user_job, mbti_detail_df, holland_result_df, job_detail_df, results, client, config, ttl_seconds, mbti_embeddings, job_embeddings)
